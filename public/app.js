...
function renderAdminAdoptions() {
  $('#admin-adoptions').innerHTML = state.admin.adoptions.length ? state.admin.adoptions.map(a => `<article class="data-row adoption-row"><div><strong>${escapeHtml(a.applicantName)}</strong><small>${escapeHtml(a.email)} • ${escapeHtml(a.phone)}</small></div><div><strong>${escapeHtml(a.animalName)}</strong><small>${formatDate(a.createdAt)}</small></div><div><small>${escapeHtml(a.reason.slice(0, 100))}${a.reason.length > 100 ? '…' : ''}</small><ol class="status-history" aria-label="Histórico da solicitação">${a.history.map(item => `<li><span>${escapeHtml(labels[item.newStatus])}</span><time>${formatDate(item.changedAt)}</time></li>`).join('')}</ol></div><div class="row-actions"><label class="sr-only" for="adoption-status-${a.id}">Status da solicitação</label><select id="adoption-status-${a.id}" class="inline-select" data-adoption-status="${a.id}" ${['aprovada','recusada'].includes(a.status) ? 'disabled' : ''}>${['recebida','em_analise','aprovada','recusada'].map(s => `<option value="${s}" ${a.status === s ? 'selected' : ''}>${labels[s]}</option>`).join('')}</select></div></article>`).join('') : '<div class="empty-state">Nenhuma solicitação recebida.</div>';
}
